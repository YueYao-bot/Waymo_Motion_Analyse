...
class DataProcessor(object):
    ...
    '''
    def __init__(self, batch_size, dataset, num_points_in_one_traj, traj_type, with_heading = False, intercept = False):
        self.batch_size = batch_size
        self.dataset = dataset  
        self.num_points_in_one_traj = num_points_in_one_traj
        self.loaded_dataset = None # the processed dataloader
        self.intercept = intercept # if True, the 0th order of polynomial is considered 
        self.traj_type = traj_type # 'ego_traj' or 'agt_traj'
        self.with_heading = with_heading # 'ego_traj' or 'agt_traj'
    ...
    def _extract_ego_trajs(self, file_path, start_idx):
        ...
        if self.intercept:
            ego_traj_temp = ego_traj_temp[:, :2] - np.mean(ego_traj_temp[:,:2], axis = 0) # trajectories are centered (0,0)
        else:
